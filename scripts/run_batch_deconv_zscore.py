"""Run group analysis on z-scored deconvolved dF/F signal, in two batches.

The deconvolved signal (delta_signal_poly_dff_deconv) is z-scored within
each session before building the trial matrix. This puts it on the same
scale as the original z-scored analysis, enabling a fair effect-size
comparison.
"""
...
def run_subject_glm_deconv_zscore(df, subject, n_shifts=N_SHIFTS, alpha=ALPHA, seed=42):
    ...
    for sess in sessions:
        ...
        # Z-score the deconvolved signal within this session
        raw = df_sess[SIGNAL_COL].values.astype(np.float64)
        mu = np.nanmean(raw)
        sd = np.nanstd(raw)
        if sd > 0:
            df_sess[SIGNAL_COL] = (raw - mu) / sd
        else:
            df_sess[SIGNAL_COL] = raw - mu  # flat signal edge case
...
print(f"Batch {batch}: {len(batch_subjects)} subjects, signal={SIGNAL_COL} (z-scored per session)")
results = []
t0 = time.time()
for si, subj in enumerate(batch_subjects):
    ...
    res = run_subject_glm_deconv_zscore(df, subj, n_shifts=5000, alpha=0.01,
...
with open(SAVE_DIR / f"_deconv_zscore_batch{batch}_results.pkl", "wb") as f:
    pickle.dump(results, f)
print(f"Saved to _deconv_zscore_batch{batch}_results.pkl")
